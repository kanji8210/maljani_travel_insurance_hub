...
class Maljani_Verification_Certificate {
    ...
    public function generate_certificate() {
        ...
        if ( ! self::user_can_view_sale( $sale ) ) {
            wp_die( 'Unauthorized access.' );
        }

        echo self::build_certificate_html( $sale ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        exit;
    }
    ...
    public static function get_sale( $sale_id ) {
        ...
        return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table WHERE id = %d", $sale_id ) );
    }

    public static function user_can_view_sale( $sale ) {
        if ( ! $sale ) {
            return false;
        }
        ...
        return $user_id > 0 && (int) $sale->agent_id === $user_id;
    }

    public static function build_certificate_html( $sale ) {
        if ( ! $sale ) {
            return '';
        }
        ...
        <head>
            ...
            <style>
                ...
                .certificate-container { max-width: 800px; margin: 40px auto; background: #fff; padding: 60px; border-radius: 4px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); position: relative; border-top: 10px solid <?php echo esc_html( $status_color ); ?>; }
                ...
                .certificate-title p { margin: 5px 0 0; font-size: 14px; font-weight: 600; color: <?php echo esc_html( $status_color ); ?>; }
                ...
                @media print {
                    ...
                    .certificate-container { margin: 0; padding: 40px; box-shadow: none; max-width: 100%; border: 1px solid #e2e8f0; border-top: 10px solid <?php echo esc_html( $status_color ); ?>; }
            ...
            <div class="certificate-container">
                ...
                <div class="footer">
                    <div class="footer-text">
                        ...
                        <p>Issued on: <?php echo esc_html( date( 'F j, Y, g:i a' ) ); ?></p>
                    </div>
                    <div class="stamp-box">
